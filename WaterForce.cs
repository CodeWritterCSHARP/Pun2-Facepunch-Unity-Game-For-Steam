...
public class WaterForce : MonoBehaviourPun
{
    ...
    private void Start()
    {
        if (GetComponent<PhotonView>() != null) view = this.photonView;
    ...
    private void AddForcePower()
    {
        ...
        foreach (Transform child in gameObject.transform.parent)
            child.GetComponent<Rigidbody2D>().AddForce(transform.up * 4);
    }
    ...
    private void OnCollisionStay2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "Water")
        {
            ...
            else
            {
                ...
                if (capsuleCollider.enabled == true && collist.Count > 4)
                {
        ...
        if (collision.gameObject.tag == "Water")
        {
            ...
            if(CanSwim == false) 
            {
                if (capsuleCollider.enabled == true && collist.Count <= 4)
                {
                    ...
                    view.RPC("GrabDisable", RpcTarget.All, true);
                }
            }
        }
    }
